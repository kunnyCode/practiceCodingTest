...
def get_next_pos(pos, board):
    ...
    for i in range(4):
        Np1x, Np1y, Np2x, Np2y = p1x+dx[i], p1y+dy[i], p2x+dx[i], p2y+dy[i]
    ...
    if p1x == p2x:
        for i in [-1, 1]:
            if board[p1x+i][p1y] == 0 and board[p2x+i][p2y] == 0:
                next_pos.append({(p1x, p1y),(p1x+i,p1y)})
                next_pos.append({(p2x, p2y),(p2x+i,p2y)})
                
    elif p1y == p2y:
        for i in [-1, 1]:
            if board[p1x][p1y+i] == 0 and board[p2x][p2y+i] == 0:
                next_pos.append({(p1x, p1y),(p1x,p1y+i)})
                next_pos.append({(p2x, p2y),(p2x,p2y+i)})
    return next_pos
...
#             if board[pos1_x][pos1_y + i] == 0 and board[pos2_x][pos2_y + i] == 0: # 왼쪽 혹은 오른쪽으로 두 칸이 모두 비어 있다면
